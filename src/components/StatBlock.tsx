...
export function StatBlock({ json }: StatBlockProps) {
  ...
  return (
    <section className="my-6 rounded-[1.6rem] border-2 border-gray-900 bg-[var(--aqs-accent-soft)]/60 p-4 dark:border-gray-100 dark:bg-[color:rgba(122,31,52,0.18)] md:p-5">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {spec.items.map((item) => (
          ...
          >
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--aqs-accent-strong)] dark:text-[var(--aqs-accent-dark)]">
              {item.label}
            </p>
