...
export function AiCitationModal({ open, citationInput, onClose }: AiCitationModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-stretch justify-center bg-black/60 p-4 no-print sm:items-center" role="dialog" aria-modal="true" aria-label="AI citation helper">
      <div className="flex max-h-[calc(100dvh-2rem)] w-full max-w-3xl flex-col overflow-hidden rounded-4xl border border-(--aqs-ink)/12 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(248,241,236,0.96))] shadow-[0_32px_100px_rgba(31,23,28,0.3)] dark:border-white/10 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.98),rgba(29,18,26,0.94))]">
        <div className="shrink-0 flex items-start justify-between gap-4 border-b border-(--aqs-ink)/10 px-6 py-5 dark:border-white/10">
          <div>
          ...
          <button type="button" onClick={onClose} aria-label="Close citation dialog" className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-(--aqs-ink)/10 bg-white text-(--aqs-ink) dark:border-white/10 dark:bg-slate-950 dark:text-white">
            <X className="h-5 w-5" />
          </button>
        </div>
